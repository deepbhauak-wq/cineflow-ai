  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 max-w-md mx-auto space-y-4">
      <div className="flex justify-between items-center border-b border-gray-800 pb-2">
        <h1 className="text-sm font-bold text-purple-400 uppercase">
          Vault Library
        </h1>
        <Link
          href="/"
      ...
      {list.length === 0 && (
        <div className="bg-gray-900 border border-gray-800 p-6 rounded-xl text-center space-y-2">
          ...
          <Link
            href="/"
            className="inline-block py-1 px-3 bg-cyan-600 rounded text-xs font-bold text-white"
          >
            Create in Studio
          </Link>
        </div>
      ...
      {list.length > 0 && (
        <div className="space-y-3">
          <button
            onClick={clearAll}
            className="w-full py-1.5 bg-red-950 border border-red-800 text-red-400 text-xs font-bold rounded"
          >
            Clear All Vault
          </button>
          {list.map((item) => (
            ...
            >
              ...
              <p className="text-[10px] text-gray-400">
                {item.scenes?.length || 0} Scenes Saved
              </p>
            </div>
          ))}
        </div>
