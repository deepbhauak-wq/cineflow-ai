    ...
    <div className="min-h-screen bg-[#07090e] text-white p-2.5 font-sans pb-20 relative selection:bg-cyan-500 selection:text-black">
      ...
      {analyzing && (
        <div className="fixed inset-0 bg-black/90 z-50 flex flex-col items-center justify-center p-4 gap-2">
          ...
          <p className="text-[11px] text-yellow-400 font-bold">AI Analyzing Video Scenes & Extracting Best Frame...</p>
        </div>
      )}
      ...
      <div className="max-w-2xl mx-auto space-y-2.5">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-2 space-y-1.5 shadow-xl">
          <div className="flex justify-between items-center px-0.5">
            ...
            <span className="text-[8px] text-green-400 font-mono">Multi-Language Ready</span>
          </div>
          <div className="w-full aspect-video rounded-lg bg-black border border-slate-800 relative flex items-center justify-center overflow-hidden">
            ...
            {thumbText && (
              <div className={`absolute left-0 right-0 px-4 text-center z-10 pointer-events-none ${
                ...
                <span className="bg-black/80 backdrop-blur-md text-yellow-300 font-black px-3 py-1.5 rounded-lg border border-yellow-500/50 text-[11px] shadow-2xl tracking-wide uppercase">
                  {thumbText}
                </span>
              </div>
            )}
        ...
        {tab === "thumb" && (
          <div className="bg-slate-900 border border-yellow-500/30 rounded-lg p-2.5 space-y-2.5 text-xs">
            <div className="flex justify-between items-center border-b border-slate-800 pb-1">
              <span className="font-bold text-yellow-400 text-[9px] uppercase">AI Video Analysis & Thumbnail Studio</span>
              <button onClick={handleAnalyzeVideo} className="px-2.5 py-1 rounded bg-yellow-500 text-black font-bold text-[9px] cursor-pointer shadow">
                ⚡ Analyze Video & Auto Generate
              </button>
            </div>
            ...
            {/* Position Picker */}
            <div>
              <span className="text-[8px] text-slate-400 block mb-1">Text Position on Thumbnail:</span>
              <div className="grid grid-cols-3 gap-1">
                {[
                  ...
                  <button key={pos.id} onClick={() => setTextPos(pos.id)} className={`py-1 rounded border text-[9px] font-bold cursor-pointer ${textPos === pos.id ? "bg-yellow-500/20 border-yellow-500 text-yellow-300" : "bg-slate-800 border-slate-700 text-slate-400"}`}>{pos.l}</button>
                ))}
              </div>
            </div>
          </div>
        ...
        {tab === "logo" && (
          <div className="bg-slate-900 border border-amber-500/30 rounded-lg p-2.5 space-y-2 text-xs">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button onClick={() => { setCustomLogo("https://images.unsplash.com/photo-1534447677768-be436bb09401?w=200"); alert("⚡ AUTO: Logo loaded!"); }} className="py-1.5 px-3 rounded bg-cyan-600 text-white font-bold text-[10px] cursor-pointer">⚡ AUTO</button>
            </div>
