...
export default function ProStudioEditor() {
  ...
  // Logo & Watermark System (Gallery / Custom Upload Support)
  ...
  const runExport = (quality) => {
    ...
    setTimeout(() => {
      setProc(false);
      setModal(false);
      alert("Master Video Downloaded (" + quality + ") with Permanent Watermark Embedded at [" + pos.toUpperCase() + "]!");
    }, 2000);
  };
    ...
    <div className="min-h-screen bg-[#07090e] text-white p-3 font-sans pb-24 relative">
      {proc && (
        <div className="fixed inset-0 bg-black/90 z-50 flex flex-col items-center justify-center p-4 gap-3">
          ...
          <p className="text-xs text-cyan-400 font-bold">Processing 4K Master Video & Permanent Watermark Embedding...</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-3 shadow-2xl">
            <div className="flex justify-between items-center border-b border-slate-800 pb-2">
              <span className="text-xs font-bold text-cyan-400 uppercase">Master Direct Exporter</span>
              <button onClick={() => setModal(false)} className="text-xs text-slate-400">✕</button>
            </div>
            <div className="space-y-2 text-xs">
              <button onClick={() => runExport("4K Cinema (60 FPS)")} className="w-full py-2.5 rounded-xl bg-cyan-500 text-black font-bold cursor-pointer">⬇ Direct Download 4K UHD</button>
              ...
              <button onClick={() => runExport("Audio WAV")} className="w-full py-2.5 rounded-xl bg-purple-500/20 text-purple-300 border border-purple-500 font-bold cursor-pointer">⬇ Download Audio Master (WAV)</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-3xl mx-auto flex items-center justify-between border-b border-slate-800/80 pb-2 mb-3">
        <div>
          <h1 className="text-xs font-bold text-white tracking-tight flex items-center gap-1.5">🎬 PRO CINEMA STUDIO</h1>
          <p className="text-[9px] text-slate-400">Direct Download • Permanent Watermark • SEO Engine</p>
        </div>
        <div className="flex items-center gap-2">
          <Link href="/" className="px-2.5 py-1 rounded-lg bg-slate-800 text-[11px] text-cyan-300 border border-slate-700">← Hub</Link>
          <button onClick={() => setModal(true)} className="px-3 py-1 rounded-lg bg-gradient-to-r from-cyan-500 to-blue-600 text-black font-bold text-[11px] shadow-lg cursor-pointer">Export / Download 💾</button>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto space-y-3">
        <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-2.5 space-y-2 shadow-xl">
          ...
          <div className="w-full aspect-video rounded-xl bg-black border border-slate-800 relative flex items-center justify-center overflow-hidden">
            ...
            {wmOn && (
              <div className={`absolute ${pos === "top-left" ? "top-2 left-2" : pos === "top-right" ? "top-2 right-2" : pos === "bottom-left" ? "bottom-2 left-2" : "bottom-2 right-2"} px-2.5 py-1 rounded-lg border text-[10px] font-bold backdrop-blur-md flex items-center gap-1.5 ${style === "gold" ? "bg-amber-950/90 border-amber-500 text-amber-300 shadow-lg" : style === "neon-cyan" ? "bg-cyan-950/90 border-cyan-500 text-cyan-300 shadow-lg" : "bg-black/90 border-slate-600 text-white"}`}>
                {customLogoUrl ? <img src={customLogoUrl} alt="" className="w-4 h-4 rounded-full object-cover"/> : <span>👑</span>}
        ...
        <div className="bg-slate-900 border border-cyan-500/30 rounded-xl p-2 flex gap-2">
          ...
          <button onClick={() => { setProc(true); setTimeout(() => { setProc(false); alert("AI Director applied!"); setPrompt(""); }, 1500); }} className="px-3 py-1 rounded-lg bg-cyan-500 text-black font-bold text-xs whitespace-nowrap cursor-pointer">Run ✨</button>
        </div>

        <div className="bg-slate-900 border border-purple-500/30 rounded-xl p-2.5 flex items-center justify-between">
          <div>
            <p className="text-[11px] font-bold text-purple-300">⭐ Cinematic Master Preset</p>
            <p className="text-[9px] text-slate-400">Locked Character • 1.5s Pause • Deep Narration • BGM -22dB</p>
          </div>
          <button onClick={() => setAutoPreset(!autoPreset)} className={`px-2.5 py-1 rounded-lg text-[10px] font-bold cursor-pointer ${autoPreset ? "bg-purple-600 text-white" : "bg-slate-800 text-slate-400"}`}>
            {autoPreset ? "ACTIVE 🟢" : "OFF"}
          </button>
        </div>
        ...
        <div className="flex gap-1 overflow-x-auto bg-slate-950 p-1 rounded-xl border border-slate-800">
          {[
            ...
            { id: "publish", label: "🔴 SEO & Copy" },
            { id: "thumbnails", label: "🖼️ Thumbnails" },
            { id: "download", label: "💾 Direct Download" }
          ].map((t) => (
            <button key={t.id} onClick={() => setTab(t.id)} className={`px-2.5 py-1 rounded-lg text-[11px] font-bold whitespace-nowrap transition cursor-pointer ${tab === t.id ? "bg-cyan-500 text-black" : "text-slate-400 hover:text-white"}`}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {tab === "watermark" && (
          <div className="bg-slate-900 border border-amber-500/20 rounded-xl p-3 space-y-2.5 text-xs">
            <div className="flex justify-between items-center border-b border-slate-800 pb-1.5">
              <span className="font-bold text-amber-400 text-[10px] uppercase">Permanent Watermark & Gallery Logo</span>
            ...
            <div className="bg-slate-950 p-2 rounded-lg border border-slate-800 flex items-center justify-between">
              <label className="px-3 py-1 rounded bg-amber-500/20 border border-amber-500/40 text-amber-300 text-[10px] font-bold cursor-pointer">
                🖼️ Upload Logo from Gallery
                <input type="file" accept="image/*" onChange={(e) => { const f = e.target.files[0]; if (f) setCustomLogoUrl(URL.createObjectURL(f)); }} className="hidden"/>
              </label>
              <span className="text-[10px] text-slate-400 font-mono">{customLogoUrl ? "✅ Custom Logo Loaded" : "Using Default"}</span>
            </div>
        ...
        {tab === "publish" && (
          <div className="bg-slate-900 border border-red-500/30 rounded-xl p-3 space-y-2 text-xs">
            <div className="flex justify-between items-center border-b border-slate-800 pb-1.5">
              <span className="font-bold text-red-400 text-[10px] uppercase">SEO Packaging & Copy Utility</span>
            ...
            <div className="grid grid-cols-4 gap-2">
              {thumbs.map((img, idx) => (
                <div key={idx} className="space-y-1 bg-slate-950 p-1.5 rounded-lg border border-slate-800">
                  ...
                  <button onClick={() => alert("Thumbnail Downloaded!")} className="w-full py-0.5 rounded bg-slate-800 text-[9px] text-slate-300 font-semibold cursor-pointer">⬇ Download</button>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === "download" && (
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 space-y-2 text-xs">
            <span className="font-bold text-cyan-400 uppercase text-[10px] block border-b border-slate-800 pb-1">One-Click Master Exporter (Permanent Watermark Included)</span>
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="bg-slate-950 p-2.5 rounded-lg border border-slate-800 space-y-1">
                ...
                <button onClick={() => alert("Audio WAV downloaded!")} className="w-full py-1.5 rounded-lg b
