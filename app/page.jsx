    ...
    <div className="min-h-screen bg-[#07090e] text-white p-4 font-sans pb-28 relative">
      ...
      {loading && (
        <div className="fixed inset-0 bg-black/90 z-50 flex flex-col items-center justify-center p-4 space-y-3">
          ...
          <p className="text-xs text-cyan-400 font-bold">Rendering Autonomous Cinema Film with Locked Audio & Continuity...</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/95 z-50 flex flex-col items-center justify-center p-4">
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-5 space-y-3 shadow-2xl">
            <div className="flex justify-between items-center">
              <span className="text-xs font-bold text-green-400">🎉 Master Video Generated Successfully</span>
            ...
            <div className="w-full aspect-video rounded-2xl overflow-hidden bg-black relative flex items-center justify-center">
              ...
              <button onClick={() => setIsPlayingCompleted(!isPlayingCompleted)} className="absolute inset-0 flex items-center justify-center bg-black/30 cursor-pointer">
                <div className="w-14 h-14 rounded-full bg-cyan-500 flex items-center justify-center text-black text-xl font-bold">
                  {isPlayingCompleted ? "❚❚" : "▶"}
                </div>
              </button>
            </div>
            <div className="flex justify-between items-center pt-2">
              <p className="text-xs text-slate-400">100% Shuddh Hindi • BGM -22dB Ducked</p>
              <Link href="/studio/editor" className="px-3 py-1.5 rounded-xl bg-cyan-500 text-black font-bold text-xs">Open in Studio Editor 🎬</Link>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        {/* Full-Size Cinematic Video Player */}
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-2 shadow-xl">
          <div className="flex justify-between items-center">
            <label className="text-xs font-semibold text-cyan-400 uppercase">🎬 Live Cinematic Video Player Preview</label>
            <span className="text-[10px] text-cyan-300 font-mono">⚡ 4K RENDER READY</span>
          </div>
          <div className="w-full aspect-video rounded-xl overflow-hidden bg-black relative flex items-center justify-center shadow-inner">
            ...
            <button onClick={() => setIsPlayingDashboard(!isPlayingDashboard)} className="absolute inset-0 flex items-center justify-center cursor-pointer">
              <div className="w-14 h-14 rounded-full bg-cyan-500/90 backdrop-blur-md flex items-center justify-center text-black text-xl font-bold shadow-xl">
                {isPlayingDashboard ? "❚❚" : "▶"}
              </div>
            </button>
          </div>
        ...
        {/* 1. Master Story Prompt */}
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-2 shadow-xl">
          <label className="text-xs font-semibold text-cyan-400 uppercase">1. Master Story Prompt & Reference Image</label>
          <textarea rows={2} value={storyPrompt} onChange={(e) => setStoryPrompt(e.target.value)} placeholder="Enter storyline here... or write AUTO to let AI create narrative." className="w-full bg-slate-800 border border-slate-700 rounded-xl p-2.5 text-xs text-white"/>
          <div className="flex items-center gap-3">
            <label className="px-3 py-1.5 rounded-xl bg-cyan-500/20 border border-cyan-500/40 text-[11px] text-cyan-300 cursor-pointer font-semibold inline-flex items-center gap-1.5">
              <span>🖼️ Gallery Upload Reference</span>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3 space-y-2 shadow-xl">
            <label className="text-xs font-semibold text-cyan-400 uppercase">5. Video Model ({videoModel})</label>
            <div className="grid grid-cols-3 gap-1">
              {["AUTO", "Veo", "Kling", "Runway", "Hailuo", "Luma"].map((m) => (
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3 space-y-2 shadow-xl">
            <label className="text-xs font-semibold text-cyan-400 uppercase">6. Story Model ({storyModel})</label>
            <div className="grid grid-cols-3 gap-1">
              {["AUTO", "Gemini", "Claude", "AutoGPT", "Fast AI", "Pro AI"].map((s) => (
      ...
      <div className="fixed bottom-4 right-5 z-50 flex items-center gap-2 bg-slate-900/95 border border-slate-700 p-1.5 rounded-full shadow-2xl backdrop-blur-lg">
        <Link href="/" className="w-9 h-9 rounded-full bg-cyan-500 text-black flex items-center justify-center text-sm font-bold border border-cyan-400 shadow-md" title="Home">🏠</Link>
        ...
        <Link href="/studio/editor" className="w-9 h-9 rounded-full bg-slate-800 text-cyan-400 flex items-center justify-center text-sm border border-slate-700 shadow-md" title="Editor">🎬</Link>
      </div>
    </div>
  );
}
