  ...
  return (
    <div className="min-h-screen bg-[#07090e] text-white p-4 font-sans pb-28 relative">
      ...
      <div className="max-w-4xl mx-auto space-y-4">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3 space-y-2">
            ...
            <div className="grid grid-cols-3 gap-1">
              {["16:9", "9:16", "21:9"].map((r) => (
                <button key={r} onClick={() => setAspectRatio(r)} className={`py-1.5 rounded-lg border text-xs ${aspectRatio === r ? "bg-cyan-500/20 border-cyan-500 text-cyan-300" : "bg-slate-800 border-slate-700 text-slate-400"}`}>{r}</button>
              ))}
            </div>
          </div>

          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3 space-y-2">
            <label className="text-xs font-semibold text-cyan-400 uppercase">4. Duration</label>
            <div className="grid grid-cols-2 gap-1">
              {["3 Min", "15 Min"].map((d) => (
                <button key={d} onClick={() => setDuration(d)} className={`py-1.5 rounded-lg border text-xs ${duration === d ? "bg-purple-500/20 border-purple-500 text-purple-300" : "bg-slate-800 border-slate-700 text-slate-400"}`}>{d}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3 space-y-2">
          <label className="text-xs font-semibold text-cyan-400 uppercase">5. Voiceover Language</label>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-1">
            {["Hindi (Pure Shuddh)", "English", "Spanish", "Arabic"].map((l) => (
              <button key={l} onClick={() => setVoiceLang(l)} className={`py-1.5 rounded-lg border text-xs ${voiceLang === l ? "bg-cyan-500/20 border-cyan-500 text-white" : "bg-slate-800 border-slate-700 text-slate-400"}`}>{l}</button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-1">
              {["Veo", "Kling"].map((m) => (
                <button key={m} onClick={() => setVideoModel(m)} className={`py-1.5 rounded-lg border text-xs ${videoModel === m ? "bg-cyan-500/20 border-cyan-500 text-cyan-300" : "bg-slate-800 border-slate-700 text-slate-400"}`}>{m}</button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-1">
              {["Gemini", "Claude"].map((s) => (
                <button key={s} onClick={() => setStoryModel(s)} className={`py-1.5 rounded-lg border text-xs ${storyModel === s ? "bg-purple-500/20 border-purple-500 text-purple-300" : "bg-slate-800 border-slate-700 text-slate-400"}`}>{s}</button>
              ))}
            </div>
          </div>
        </div>

        <button onClick={handleGenerate} className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-cyan-500 to-blue-600 font-bold text-xs text-black cursor-pointer shadow-lg">
          🚀 GENERATE AUTONOMOUS CINEMA FILM
        </button>
      </div>

      <div className="fixed bottom-3 right-4 z-50">
        <Link href="/studio/editor" className="w-8 h-8 rounded-full bg-cyan-500 text-black flex items-center justify-center font-bold text-xs shadow-lg">→</Link>
      </div>
    </div>
  );
}
