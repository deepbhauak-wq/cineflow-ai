...
export default function CineFlowProStudio() {
  ...
  const saveEdit = (id) => {
    setScenes(prev => prev.map(s => s.id === id ? {...s, desc: editPrompt} : s));
    setEditingId(null);
    setStatusMsg("✅ Scene Updated!");
  };
            ...
            <div key={sc.id} className="bg-[#0b1222] border border-slate-800 p-4 rounded-xl space-y-2">
              ...
              {editingId === sc.id ? (
                <div className="space-y-2">
                  ...
                  <button onClick={() => saveEdit(sc.id)} className="w-full py-2 bg-emerald-600 text-[10px] font-bold rounded">SAVE CHANGES</button>
                </div>
              ) : (
                <>
                  <p className="text-[11px] text-slate-300">{sc.desc}</p>
                  <button onClick={() => startEdit(sc)} className="w-full py-2 bg-slate-800 text-[10px] font-bold rounded">✏️ EDIT PROMPT</button>
                </>
              )}
            </div>
          ))}
