      ...
      <div className="min-h-screen w-full bg-[#0a0d14] flex items-center justify-center p-4 text-white font-sans">
        <div className="w-full max-w-sm rounded-3xl bg-[#0f1422]/95 border border-slate-800/80 p-8 shadow-2xl backdrop-blur-xl flex flex-col items-center">
          ...
          <div className="w-full flex flex-col gap-3">
            <button
              type="button"
              onClick={() => handleLogin("google.user@gmail.com")}
              className="w-full py-3 px-4 rounded-xl bg-[#141b2d] hover:bg-[#1c263f] border border-slate-700/60 flex items-center justify-center gap-3 transition text-sm font-medium cursor-pointer"
            >
            ...
            <button
              type="button"
              onClick={() => handleLogin("facebook.user@fb.com")}
              className="w-full py-3 px-4 rounded-xl bg-[#141b2d] hover:bg-[#1c263f] border border-slate-700/60 flex items-center justify-center gap-3 transition text-sm font-medium cursor-pointer"
            >
            ...
            <button
              type="button"
              onClick={() => handleLogin("instagram.user@insta.com")}
              className="w-full py-3 px-4 rounded-xl bg-[#141b2d] hover:bg-[#1c263f] border border-slate-700/60 flex items-center justify-center gap-3 transition text-sm font-medium cursor-pointer"
            >
              <svg className="w-5 h-5 flex-shrink-0" viewBox="0 0 24 24">
                <defs>
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleLogin(email); }} className="w-full space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full bg-[#141b2d] border border-slate-700/60 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-[#141b2d] border border-slate-700/60 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500"
                />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-2.5 text-xs text-slate-400 hover:text-white">
                  {showPassword ? "Hide" : "Show"}
                </button>
              </div>
            </div>

            <button type="submit" className="w-full mt-2 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 font-semibold text-sm shadow-lg shadow-cyan-500/25 hover:opacity-95 transition">
              SIGN IN
            </button>
          </form>
