      ...
      const scList = [
        {
          id: 1,
          title: "Scene 1",
          desc: "योद्धा घने जंगल में आगे बढ़ता है।",
          camera: cameraRig,
        ...
        },
        {
          id: 2,
          title: "Scene 2",
          desc: "मंदिर का विशाल दरवाजा खुलता है।",
          camera: "JCB Mechanical Jib",
          voice: "द्वार खुलते ही दिव्य प्रकाश फैला...",
          bgm: "Ancient Mystery Horns",
    ...
    return (
      <div className="min-h-screen bg-[#070b14] text-white flex items-center justify-center p-4">
        <div className="w-full max-w-xs bg-[#0b1222] p-6 rounded-2xl border border-slate-800 text-center space-y-4">
          ...
          <button
            onClick={() => { setIsLoggedIn(true); notify("Logged in!"); }}
            className="w-full py-2.5 bg-cyan-600 font-bold text-xs rounded-xl uppercase text-white"
          >
            Enter Studio
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#070b14] text-slate-100 p-4 pb-20 max-w-md mx-auto space-y-3 font-sans">
      <header className="flex justify-between items-center border-b border-slate-800 pb-2">
        <div>
          <h1 className="text-xs font-black text-cyan-400">CINEFLOW VAULT</h1>
          <span className="text-[9px] text-emerald-400">{credits} Credits</span>
        </div>
        <button
          onClick={() => setIsLoggedIn(false)}
          className="text-[10px] bg-red-950 text-red-400 px-2 py-0.5 rounded font-bold"
        >
          Logout
        </button>
      </header>
      ...
      {statusMsg && (
        <div className="fixed top-3 left-1/2 -translate-x-1/2 bg-cyan-950 text-cyan-300 border border-cyan-500 px-3 py-1 rounded-full text-[10px] font-bold z-50">
          {statusMsg}
        </div>
      )}
        ...
        <div className="p-2 bg-[#0b1222] border border-slate-800 rounded-xl space-y-1">
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-1">
            {savedProjects.map((p) => (
              <button
                key={p.id}
                onClick={() => { setScenes(p.scenes); notify("Project Loaded!"); }}
                className="p-1.5 bg-[#060a14] border border-slate-800 rounded-lg text-left shrink-0 text-[10px] text-slate-300"
              >
                {p.name}...
              </button>
            ))}
      ...
      <div className="p-2.5 bg-[#0b1222] border border-slate-800 rounded-xl space-y-1">
        ...
        <textarea
          ...
          rows={2}
          className="w-full bg-[#060a14] p-2 text-xs text-white rounded outline-none border border-slate-800"
        />
      </div>
          ...
          <div className="grid grid-cols-2 gap-1">
            {["JCB Crane", "Drone"].map((c) => (
              <button
                ...
                className={`p-1 text-[9px] font-bold rounded ${cameraRig === c ? "bg-amber-950 text-amber-300 border border-amber-500" : "bg-[#060a14]"}`}
              >
                {c}
              </button>
            ))}
          ...
          <div className="flex gap-1">
            {["16:9", "9:16"].map((r) => (
              <button
                key={r}
                onClick={() => setRatio(r)}
                className={`flex-1 p-1 text-[9px] font-bold rounded ${ratio === r ? "bg-cyan-950 text-cyan-300 border border-cyan-500" : "bg-[#060a14]"}`}
              >
                {r}
              </button>
            ))}
      ...
      <button
        ...
        className="w-full py-3 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl font-black text-xs uppercase text-white shadow-lg"
      >
        🚀 Generate & Save Suite
      </button>
                ...
                {editingId === sc.id ? (
                  <div className="space-y-1.5 bg-[#060a14] p-2 rounded-lg border border-cyan-500">
                    <textarea
                      ...
                      rows={2}
                      className="w-full bg-[#0b1222] p-1.5 text-xs text-white rounded outline-none border border-slate-700"
                    />
                    <div className="flex gap-1">
                      <input
                        type="text"
                    ...
                    <div className="flex gap-1 pt-1">
                      <button
                        ...
                        className="flex-1 py-1 bg-emerald-600 text-[9px] font-bold text-white rounded"
                      >
                        💾 Save
                      </button>
                      <button
                  ...
                  <>
                    ...
                    <div className="flex gap-1.5 pt-1">
                      <button
                        onClick={() => notify(`🔄 Scene ${sc.id} Re-Rolled!`)}
                        className="flex-1 py-1 bg-cyan-950 border border-cyan-500 text-cyan-300 text-[9px] font-bold rounded"
                      >
                        🔄 Re-Roll
                      </button>
                      <button
                        ...
                        className="px-3 py-1 bg-slate-800 text-slate-200 text-[9px] font-bold rounded"
                      >
                        ✏️ Edit
                      </button>
                    </div>
