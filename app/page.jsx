...
export default function CineFlow() {
  ...
  const [p, setP] = useState("योद्धा मंदिर की खोज में है।");
  ...
  const gen = () => {
    ...
    setTimeout(() => {
      setCr(c => c - 10);
      setScs([
        { id: 1, t: "Scene 1", desc: "योद्धा जंगल में प्रवेश करता है।", c: "Drone" },
        { id: 2, t: "Scene 2", desc: "मंदिर का मुख्य द्वार खुलता है।", c: "Dolly" }
      ]);
      setDone(true);
      pop("✅ Done!");
    }, 1000);
  ...
  return (
    <div className="min-h-screen bg-[#070b14] text-slate-100 p-4 pb-16 max-w-md mx-auto space-y-3">
      ...
      <div className="p-2 bg-[#0b1222] border border-slate-800 rounded-xl space-y-1">
        <label className="text-[9px] font-bold text-cyan-400">1. Story Prompt</label>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="p-2 bg-[#0b1222] border border-slate-800 rounded-xl">
          <label className="text-[8px] font-bold text-cyan-400">2. Ratio</label>
        ...
        <div className="p-2 bg-[#0b1222] border border-slate-800 rounded-xl">
          <label className="text-[8px] font-bold text-indigo-400">3. Timeline</label>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="p-2 bg-[#0b1222] border border-slate-800 rounded-xl">
          <label className="text-[8px] font-bold text-purple-400">4. Story AI</label>
          <div className="flex gap-1 pt-1">{["Auto", "Gemini"].map(x => <button key={x} onClick={() => setSm(x)} className={`flex-1 p-1 text-[9px] font-bold rounded ${sm === x ? "bg-purple-950 text-purple-300" : "bg-[#060a14]"}`}>{x}</button>)}</div>
        </div>
        <div className="p-2 bg-[#0b1222] border border-slate-800 rounded-xl">
          <label className="text-[8px] font-bold text-cyan-400">5. Video AI</label>
          <div className="flex gap-1 pt-1">{["Veo", "Kling"].map(x => <button key={x} onClick={() => setVm(x)} className={`flex-1 p-1 text-[9px] font-bold rounded ${vm === x ? "bg-cyan-950 text-cyan-300" : "bg-[#060a14]"}`}>{x}</button>)}</div>
        </div>
      </div>

      <div className="p-2 bg-[#0b1222] border border-slate-800 rounded-xl">
        <label className="text-[9px] font-bold text-amber-400">6. Style</label>
        <div className="grid grid-cols-2 gap-1 pt-1">{["Cinematic Epic", "3D Animation"].map(x => <button key={x} onClick={() => setStyle(x)} className={`p-1 text-[9px] font-bold rounded ${style === x ? "bg-[#1f1910] border border-amber-500 text-amber-300" : "bg-[#060a14]"}`}>{x}</button>)}</div>
      </div>
              ...
              {eid === s.id ? (
                ...
                <>
                  ...
                  <div className="flex gap-1.5 pt-0.5">
                    <button onClick={() => pop(`🔄 Regenerating Scene ${s.id}...`)} className="flex-1 py-1 bg-cyan-950 border border-cyan-500 text-cyan-300 text-[9px] font-bold rounded">🔄 Regenerate</button>
