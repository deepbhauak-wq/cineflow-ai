...
export default function CineFlowProStudio() {
  ...
  const handleGenerate = () => {
    if (credits < 10) return notify("⚠️ क्रेडिट्स समाप्त! प्लान अपग्रेड करें।");
    setPipelineState("generating");
    notify("🚀 AI फिल्म पाइपलाइन रेंडरिंग जारी...");
    setTimeout(() => {
      setCredits((prev) => prev - 10);
      setScenes([
        { id: 1, title: "Scene 01: The Storm Begins", desc: "योद्धा घने जंगल में प्रवेश करता है, रात का समय और तेज बारिश।", camera: "Wide Drone", voice: "रात बहुत अंधेरी थी...", status: "Ready" },
        { id: 2, title: "Scene 02: Ancient Temple Discovery", desc: "प्राचीन मंदिर के दरवाजे पर रहस्यमयी सुनहरी रोशनी का दिखना।", camera: "Dolly In", voice: "सामने पुराना दरवाजा दिखा...", status: "Ready" },
        { id: 3, title: "Scene 03: The Awakening", desc: "दरवाजा धीरे-धीरे खुलता है और दिव्य ऊर्जा निकलती है।", camera: "Low Angle", voice: "कदम आगे बढ़ाए...", status: "Ready" }
      ]);
  ...
  const handleSaveScene = (id) => {
    setScenes((prev) => prev.map((s) => (s.id === id ? { ...s, desc: editPromptText, status: "Updated" } : s)));
      ...
      <div className="min-h-screen bg-[#070b14] text-slate-100 flex items-center justify-center p-4">
        <div className="w-full max-w-sm bg-[#0b1222] p-6 rounded-3xl border border-slate-800 shadow-2xl space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => { setIsLoggedIn(true); notify("Google से लॉगिन पूरा हुआ!"); }} type="button" className="py-2.5 rounded-xl bg-white text-slate-900 font-bold text-xs">🌐 Google</button>
            <button onClick={() => { setIsLoggedIn(true); notify("Facebook से लॉगिन पूरा हुआ!"); }} type="button" className="py-2.5 rounded-xl bg-[#1877F2] text-white font-bold text-xs">🔵 Facebook</button>
          </div>
    ...
    <div className="min-h-screen bg-[#070b14] text-slate-100 font-sans pb-24">
      ...
      <main className="max-w-xl mx-auto px-4 pt-4 space-y-4">
        ...
        <div className="p-3.5 rounded-2xl bg-[#0b1222] border border-slate-800 space-y-1.5">
          ...
          <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={3} className="w-full bg-[#060a14] border border-slate-800 rounded-xl p-2.5 text-xs text-slate-200 outline-none focus:border-cyan-500 resize-none" />
        </div>
        ...
        <div className="p-3.5 rounded-2xl bg-[#0b1222] border border-slate-800 space-y-1.5">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {["16:9", "9:16", "21:9", "4:3", "1:1", "Auto"].map((r) => (
              <button key={r} onClick={() => setRatio(r)} className={`p-2 rounded-xl border text-xs font-bold transition-all ${ratio === r ? "bg-cyan-950 border-cyan-500 text-cyan-300" : "bg-[#060a14] border-slate-800 text-slate-400"}`}>{r}</button>
            ))}
          </div>
        </div>
        ...
        <div className="p-3.5 rounded-2xl bg-[#0b1222] border border-slate-800 space-y-1.5">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {["3 Min", "15 Min", "20 Min", "30 Min", "60 Min", "Custom"].map((d) => (
              <button key={d} onClick={() => setDuration(d)} className={`p-2 rounded-xl border text-xs font-bold transition-all ${duration === d ? "bg-indigo-950 border-indigo-500 text-indigo-300" : "bg-[#060a14] border-slate-800 text-slate-400"}`}>{d}</button>
            ))}
          </div>
        </div>
          ...
          <div className="p-3 rounded-2xl bg-[#0b1222] border border-slate-800 space-y-1">
            ...
            <div className="grid grid-cols-2 gap-1">
              {["Auto", "GPT", "Gemini", "Claude"].map((m) => (
                <button key={m} onClick={() => setStoryModel(m)} className={`py-1.5 text-[10px] font-bold rounded-lg border transition-all ${storyModel === m ? "bg-purple-950 border-purple-500 text-purple-200" : "bg-[#060a14] border-slate-800 text-slate-400"}`}>{m}</button>
              ))}
            </div>
          </div>
          <div className="p-3 rounded-2xl bg-[#0b1222] border border-slate-800 space-y-1">
            ...
            <div className="grid grid-cols-2 gap-1">
              {["Veo", "Kling", "Runway", "Hailuo"].map((vm) => (
                <button key={vm} onClick={() => setVideoModel(vm)} className={`py-1.5 text-[10px] font-bold rounded-lg border transition-all ${videoModel === vm ? "bg-cyan-950 border-cyan-500 text-cyan-200" : "bg-[#060a14] border-slate-800 text-slate-400"}`}>{vm}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="p-3.5 rounded-2xl bg-[#0b1222] border border-slate-800 space-y-1.5">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {[
              ...
              <button key={st.id} onClick={() => setStyle(st.id)} className={`p-2 rounded-xl border text-xs font-bold flex items-center justify-between transition-all ${style === st.id ? "bg-[#1f1910] border-amber-500 text-amber-300" : "bg-[#060a14] border-slate-800 text-slate-400"}`}>
        ...
        <div className="p-3.5 rounded-2xl bg-[#0b1222] border border-slate-800 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {[
              ...
              <button key={p.id} onClick={() => { setPlan(p.id); setCredits(p.cr); notify(`Plan: ${p.name}`); }} className={`p-2.5 rounded-xl border text-center transition-all ${plan === p.id ? "border-emerald-500 bg-emerald-950/30 scale-[1.02]" : "border-slate-800 bg-[#060a14] text-slate-400"}`}>
                ...
                {editingSceneId === sc.id ? (
                  <div className="space-y-2 bg-[#060a14] p-3 rounded-xl border border-cyan-500">
                    ...
                    <div className="flex gap-2 pt-1">
                      <button onClick={() => handleSaveScene(sc.id)} className="flex-1 py-2 bg-emerald-600 hover:bg-emerald-500 text-[10px] font-black text-white uppercase rounded-lg">💾 Save & Re-render</button>
                  ...
                  <>
                    <p className="text-xs text-slate-300 bg-[#060a14] p-2.5 rounded-xl border border-slate-800 leading-relaxed">{sc.desc}</p>
                    ...
                    <div className="flex gap-2 pt-1">
                      ...
                      <button onClick={() => handleOpenEdit(sc)} className="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-[10px] font-black uppercase">✏️ Edit Prompt</button>
                    </div>
                  </>
                )}
              </div>
