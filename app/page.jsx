...
export default function CineFlow() {
  ...
  const [p, setP] = useState("योद्धा मंदिर की खोज कर रहा है।");
  ...
  const gen = () => {
    ...
    setTimeout(() => {
      setCr(c => c - 10);
      setScs([{ id: 1, t: "Scene 1", desc: "योद्धा जंगल में आगे बढ़ता है।" }, { id: 2, t: "Scene 2", desc: "मंदिर का द्वार खुलता है।" }]);
      setDone(true);
      pop("✅ Done!");
    }, 1000);
  };
  ...
  return (
    <div className="min-h-screen bg-[#070b14] text-slate-100 p-4 pb-16 max-w-md mx-auto space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="flex gap-1">{["16:9", "9:16"].map(x => <button key={x} onClick={() => setR(x)} className={`flex-1 p-1 text-[9px] font-bold rounded ${r === x ? "bg-cyan-950 text-cyan-300 border border-cyan-500" : "bg-[#060a14]"}`}>{x}</button>)}</div>
        <div className="flex gap-1">{["3 Min", "30 Min"].map(x => <button key={x} onClick={() => setD(x)} className={`flex-1 p-1 text-[9px] font-bold rounded ${d === x ? "bg-indigo-950 text-indigo-300 border border-indigo-500" : "bg-[#060a14]"}`}>{x}</button>)}</div>
      </div>
      <button onClick={gen} className="w-full py-2 bg-cyan-600 rounded-xl font-bold text-xs uppercase text-white">🚀 Generate</button>
      {done && (
            ...
            <div key={s.id} className="bg-[#0b1222] border border-slate-800 p-2.5 rounded-xl space-y-1.5">
              ...
              {eid === s.id ? (
                <div className="space-y-1">
                  ...
                  <div className="flex gap-1"><button onClick={() => { setScs(scs.map(x => x.id === s.id ? { ...x, desc: etxt } : x)); setEid(null); pop("Saved!"); }} className="flex-1 py-1 bg-emerald-600 text-[9px] font-bold rounded">Save</button><button onClick={() => setEid(null)} className="px-2 py-1 bg-slate-800 text-[9px] rounded">Cancel</button></div>
                </div>
              ) : (
                <div className="flex justify-between items-center">
                  <p className="text-[11px] text-slate-300">{s.desc}</p>
                  <button onClick={() => { setEid(s.id); setEtxt(s.desc); }} className="px-2 py-1 bg-slate-800 text-[9px] font-bold rounded">✏️ Edit</button>
                </div>
              )}
            </div>
          ))}
